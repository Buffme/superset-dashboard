...
const DashboardPage: FC = () => {
  ...
  const { dashboardId } = extractUrlParams('regular');
  // console.log('idOrSlug11', idOrSlug, dashboardId)
  const [isHydrated, setHydrated] = useState(false);
  const dashboardResource = useDashboard(dashboardId);
  const chartsResource = useDashboardCharts(dashboardId);
  const datasetsResource = useDashboardDatasets(dashboardId);
  const error = [dashboardResource, chartsResource, datasetsResource].find(
    resource => resource.status === ResourceStatus.ERROR,
  )?.error;

  useEffect(() => {
    // console.log('useEffect', dashboardResource)
