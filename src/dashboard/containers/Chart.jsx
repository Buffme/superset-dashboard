...
function mapStateToProps(
  ...
  const { id } = ownProps;
  const slice = sliceEntities.slices[id];
  ...
  const { whisker_options, viz_type, colorPicker } = formData
  if (viz_type === 'box_plot') {
    formData.whiskerOptions = whisker_options
  }
  if (viz_type === 'world_map' && !colorPicker) {
    formData.colorPicker = {r: 0, g: 0, b: 0}
  }
  // formData.datasource = datasource;
  // console.log('formData', formData, datasource, slice, chart, datasources)
  return {
    chart,
    datasource,
    slice,
    timeout: dashboardInfo.common.conf && dashboardInfo.common.conf.SUPERSET_WEBSERVER_TIMEOUT | '',
    ...
    maxRows: common.conf && common.conf.SQL_MAX_ROW,
  };
}
