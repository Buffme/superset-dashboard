...
export function getURIDirectory(endpointType = 'base') {
  ...
  if (
    ...
    return '/api/backend/superset/export_json/';
  }
  if (
    ['full', 'json', 'query', 'results', 'samples'].includes(
      endpointType,
    )
  ) {
    return '/api/backend/superset/explore_json/';
  }
...
export function postForm(url, payload, target = '_blank') {
  ...
  const tokenData = document.createElement('input');
  tokenData.type = 'hidden';
  tokenData.name = 'token';
  tokenData.value = cookie('token') || ''
  hiddenForm.appendChild(tokenData);
  console.log('postForm', hiddenForm)
...
export const exportChart = ({
  ...
  if (shouldUseLegacyApi(formData)) {
    ...
    console.log('getExploreUrl', formData)
    payload = formData;
  } else {
    url = '/api/backend/v1/chart/data/export';
    console.log('buildV1ChartDataPayload')
