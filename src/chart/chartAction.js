...
const v1ChartDataRequest = async (
  ...
  const url = getChartDataUri({
    path: '/api/backend/v1/chart/data',
    qs,
    allowDomainSharding,
  }).toString();
  const token = cookie('token') || ''
  ...
  return async dispatch => {
    ...
    const chartDataRequestCaught = chartDataRequest
      .then(({ response, json }) => {
        ...
        if (json.code === '10000004') {
          console.log(json)
          parent.postMessage( { responseCode: json.code }, '*');
          return
        }
        if (!isFeatureEnabled(FeatureFlag.GLOBAL_ASYNC_QUERIES)) {
          // deal with getChartDataRequest transforming the response data
          const result = 'result' in json.data ? json.data.result[0] : json.data;
          // console.log('22222', isFeatureEnabled(FeatureFlag.GLOBAL_ASYNC_QUERIES), response, json, result)
      ...
      .then(queriesResponse => {
        console.log('222222', queriesResponse)
        queriesResponse && queriesResponse.forEach(resultItem =>
          dispatch(
      ...
      .catch(response => {
        console.log('2222223333', response)
        if (isFeatureEnabled(FeatureFlag.GLOBAL_ASYNC_QUERIES)) {
...
export function refreshChart(chartKey, force, dashboardId) {
  return (dispatch, getState) => {
    // console.log('SUPERSET_WEBSERVER_TIMEOUT', dashboardId)
    const chart = (getState().charts || {})[chartKey];
    const timeout = getState().dashboardInfo.common.conf && getState().dashboardInfo.common.conf.SUPERSET_WEBSERVER_TIMEOUT || '';
